...
// DeleteOne 删除一条
func (d *Dao) DeleteOne(ctx context.Context, tableName string, selector bson.M) (objectID primitive.ObjectID, err error) {
	//打印日志
	lg := utils.GetDefaultLogger()
...
// Update 更新
func (d *Dao) Update(ctx context.Context, tableName string, selector bson.M, update bson.M) (objectID primitive.ObjectID, err error) {
	//打印日志
	lg := utils.GetDefaultLogger()
